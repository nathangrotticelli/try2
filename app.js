...
app.post('/unliked', function(req,res){
 ...
 // "notifications" : {message:message}
  WatchSchema.update({'watchesIndex.watchName': watch.watchName},{'$pull': {'watchesIndex.$.watchLikes': {'username':username}} },function(err1){
              if(err1){
                    console.log(err1);
              }else{
                console.log('watch unlike updated.');
                 WatchSchema.update({'users.username': username},{'$pull': {'users.$.likes': watch.watchName} },function(err2){
                      if(err2){
                          ...
                          console.log('user unlike updated.');
                          res.json(200);
                      }
                  });
              }
      ...
      if(err){
        ...
        // res.json({worked1: "user pic updated."});
         WatchSchema.update({'watchLikes.username': req.body.username},
            {'$set': {'watchLikes.$.userPic': req.body.userPic}}, function(err,worked2) {
              if(err){
                    console.log(err);
              }else{
...
var dpTransporter = nodemailer.createTransport({
    service: 'Gmail',
    auth: {
...
app.post('/userContactFormMessage',function(req,res){
  ...
  dpTransporter.sendMail(mailOptions2, function(error, info){
      if(error){
          console.log(error);
      }
      else{
